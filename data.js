// ======================== data.js — 静态数据与号码属性预计算 v3.6.0 ========================
// 新增：自动跨年生肖算法，基准年 2024 = 龙年
(function () {
  "use strict";
  ...
  const BASE_YEAR = 2024; // 基准年：2024 = 龙年
  ...
  function generateShengxiaoMap(year) {
    const taiSuiIdx = ((year - BASE_YEAR) % 12 + 12) % 12; // 当年太岁在序列中的索引
    const map = {};
    for (let i = 0; i < 12; i++) {
      const offset = (taiSuiIdx - i + 12) % 12; // 该生肖相对于太岁的偏移
      const start = offset + 1; // 起始号码（1~12）
      const nums = [];
      for (let k = 0; k < 5; k++) {
        const num = start + k * 12;
        if (num <= 49) nums.push(num); // 只取 1~49
      }
